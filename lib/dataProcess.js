...
let findMeaning = (params, next) => {
    let {
        flist,
        flistWT
    } = createArrDataForNlp(params.listTopic, params.listAttr);
    let dataArr = params.dataFor;
    dataCompare(flist, dataArr, 0.50, "", (err, succ) => {
        ...
        dataF.forEach((finalItm) => {
            ...
            let subData = "";
            if(dataArr[finalItm[4]].includes("?")){
                subData = dataArr[finalItm[4] + 1]
            }
            dataObjList.push({
                indexTopic: finalItm[0],
                ...k,
                __accurate: finalItm[1],
                type: finalItm[2],
                data: finalItm[3],
                subData
            });
...
let getHtml = (params, next) => {
    request.get(params.trustedLinks[0], function (error, response, body) {
        ...
        var dataFor = htmlToText.fromString(body, {
            wordwrap: false,
            hideLinkHrefIfSameAsText: false,
            ignoreHref: true,
            ignoreImage: true
        });
        dataFor = dataFor.split("\n\n");
        params.dataFor = dataFor;
